...
def holidaytag(currentrow):
    holiday="Not"
    winterbreakstart=datetime.datetime(2018,12,20)
    ...
    date_time.replace(2019)
    if (date_time<=winterbreakend) and (date_time>=winterbreakstart):
...
def clean_csv(filepath):
    ...
    df['Tag']=np.nan
    df['Duration']=np.nan
    df2=pd.DataFrame(columns=['Activity','Tag','Start-time','End-time','Duration','Part of day','Weekday','Holiday',"Hours"])
    for i,row in df.iterrows():
        s=str(row['Activity'])
        start_time = str(row['Start-time'])
        end_time = str(row['End-time'])
        updatetag="Miscellaneous"
        ...
        hour=hourfunction(duration)
        data=pd.DataFrame({'Activity':[s],'Tag':[updatetag],'Start-time':[start_time],'End-time':[end_time],'Duration':[duration],'Part of day':[timespan],'Weekday':[date_time],'Holiday':[holiday],'Hours':[hour]}) # tag
        df2=df2.append(data)
    return df2
...
for i in keywords:
    tagtimecharts(df1, i)
for i2 in keywords2:
    tagtimecharts(df2,i2)
keywords3=df3['Tag'].unique()
for i3 in keywords3:
    tagtimecharts(df3,i3)'''


# In[87]:


def tagweekdaycharts(data,keyword):
...
keywords2=df2['Tag'].unique()
for i2 in keywords2:
    tagweekdaycharts(df2,i2)
keywords3=df3['Tag'].unique()
for i3 in keywords3:
    tagweekdaycharts(df3,i3)'''
